...
import {
  ...
  saveAdminMaterial, deleteAdminMaterial,
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-orbit-electric/25 bg-orbit-electric/[0.03] p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div><label className={label}>Capa (URL)</label><input className={input} value={form.coverUrl} onChange={(e) => set("coverUrl", e.target.value)} /></div>
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        {course.modules.map((m) => (
          <ModuleBlock key={m.id} courseId={course.id!} module={m} onChange={onChange} onEditLesson={(lesson) => setLessonEditor({ moduleId: m.id!, lesson })} />
        ))}
    ...
    <div className="rounded-xl border border-white/10 bg-white/[0.02]">
      ...
      {open && (
        <div className="space-y-1.5 border-t border-white/10 p-3">
          {mod.lessons.map((l) => (
            <div key={l.id} className="flex items-center justify-between gap-2 rounded-lg bg-black/30 px-3 py-2">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
              ...
              <button className="text-xs text-orbit-electric hover:underline" onClick={() => onEditLesson(l)}>Abrir</button>
            </div>
          ))}
      ...
      <div className="flex h-full w-full max-w-2xl flex-col border-l border-white/10 bg-[#0a0c14]">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-4">
          ...
          <section className="space-y-2 rounded-xl border border-white/10 p-3">
            <div className="flex items-center justify-between"><h3 className="text-xs font-bold uppercase tracking-wider text-white/50">Materiais</h3><button className={btnGhost} onClick={addMaterial}><Plus className="size-4" />Adicionar</button></div>
