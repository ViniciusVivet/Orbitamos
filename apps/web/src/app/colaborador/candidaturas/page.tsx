...
import { getMyApplications, withdrawApplication, type ApplicationStatus, type JobApplication } from "@/lib/api";
...
export default function ColaboradorCandidaturas() {
  ...
  const withdraw = async (item: JobApplication) => {
    if (!window.confirm(`Retirar sua candidatura para ${item.jobTitle}?`)) return;
    try { await withdrawApplication(item.id); setItems((current) => current.map((candidate) => candidate.id === item.id ? { ...candidate, status: "withdrawn" } : candidate)); }
    catch (e) { setError(e instanceof Error ? e.message : "Erro ao retirar candidatura"); }
  };
